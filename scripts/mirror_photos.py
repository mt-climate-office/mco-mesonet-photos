...
def main() -> None:
    ...
    log.info(f"{len(tasks):,} new photos to process  ({len(existing):,} already in manifest)")
    ...
    log.info(f"Downloads: {len(download_ok):,} ok, {len(download_failed):,} failed")
    ...
    total_failed = len(download_failed) + len(convert_failed) + len(upload_failed)
    log.info(f"Done. {len(new_rows):,} uploaded, {total_failed:,} failed.")
    if total_failed:
        sys.exit(1)
